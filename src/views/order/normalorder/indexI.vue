...
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      goToCreateOrder() {
        //  alert('点击了卡片')
        this.$router.push('/promotion/createOrder?storeType=3')
      },
      /** 恢复执行单个订单 */
      handleResume(row) {
        this.$modal.confirm('是否确认恢复执行订单编号为"' + row.orderNo + '"的数据项？').then(function() {
          // 这里假设订单状态4是暂停状态，恢复后改为3（执行中），具体数值请根据实际业务调整
          const data = {
            id: row.id,
          toStatus: 4, // 设置为执行状态
          status: 6    // 从暂停状态恢复
      ...
      continueBatchResume(ordersToProcess) {
        ...
        this.$confirm(`是否批量恢复选中的 "${ordersToProcess.length}" 条暂停的订单？`).then(() => {
          ...
          this.getList();
          this.$modal.msgSuccess("批量恢复执行成功");
        }).catch(() => {
        }).finally(() => {
          this.loading = false;
        });
